...
def LookUpTableEntryAdder(
    ...
    for nu_kap1,nu_kapt,nu_kap2,COIlambda in tqdm(LambdaTripleList,position=0,leave=True):
...
#        G_ASE=G_ASE+h*nu/2*(10**(AmplifierGainList[iLink][0]/10+AmplifierNoiseFigureList[iLink][0]/10)-1)*LinkNumSpanList[iLink]*2
        TotalNoise_Var+=h*nu/2*(10**(_ampgain_/10+LinkSpec['AmpNF']/10)-1)*LinkSpec['Nspan']*2
