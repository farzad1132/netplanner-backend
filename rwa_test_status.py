...
#         print(demand)
#         print('------')
id_info = run_rwa(physical_topology=physical_topology.dict(), cluster_info=cluster_info.dict(),
                                grooming_result=grooming_result.dict(), rwa_form=rwa_form.dict())
while True:
    report = status_check(id_info)
    print(json.dumps(report.dict(), indent=4))
    print()
    time.sleep(0.75)