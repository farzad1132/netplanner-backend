...
from clusters.schemas import ClusterDict, ClusterId, ClusterIn, ClusterOut
from clusters.utils import (check_cluster_name_conflict,
                            check_pt_cluster_compatibility,
                            cluster_list_to_cluster_dict)
...
def create_cluster(clusters: List[ClusterIn], project_id: str = Body(...),
    ...
    for cluster in clusters:
        check_pt_cluster_compatibility(
            pt=project.physical_topology.data, cluster=cluster.data)
        ...
        cluster_record = ClusterModel(
            name=cluster.name, data=cluster.data.dict())
...
def read_cluster(cluster_id: str,
    ...
    if (cluster := db.query(ClusterModel)
        ...
        raise HTTPException(
            status_code=404, detail="no cluster in project with given id")
