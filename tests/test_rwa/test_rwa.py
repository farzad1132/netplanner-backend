...
def test_authorization():
    ...
    data = {"username": "amir",
            "password": "1234"}
    response = client.post(PREFIX + "/v2.0.0/users/login",
...
def test_run_rwa_v2_0_0():
    ...
    # checking grooming
    grooming_id_list = GroomingIdList(
        **{"grooming_id_list": [GLOBAL["grooming_id"]]}).dict()
    ...
    response = client.post(PREFIX + "/v2.0.0/algorithms/rwa/start",
                           headers=HEADER,
                           params={
                               "grooming_id": GLOBAL["grooming_id"], "project_id": "kerman"},
                           json=rwa_form)
...
def test_rwa_check_v2_0_0():
    response = client.post(PREFIX + "/v2.0.0/algorithms/rwa/check",
                           headers=HEADER,
                           json={
                               "rwa_id": GLOBAL["rwa_id"]
                           })

    assert response.status_code == 200
    status = response.json()
    assert status["current_stage_info"] == "Finished successfully."
